...
def run(now=None,manual=False):
 now=now or pd.Timestamp.now(tz='Asia/Kolkata'); holidays=nse_fo_holidays(now.year)
 ...
 if s['gate'] and not already:
  try:
   qty={x[0]:x[2] for x in NODIP_LEGS};prices={};srcs={}
   for lab,side,q in NODIP_LEGS:prices[lab],srcs[lab]=entry_price(best_quote(x,side,s['strikes'][lab]),q,2.0)
   costs=entry_cost('NIFTY',prices,qty,_lot(),now.date());pid=stable_id({'signal_id':sid,'prices':prices});pos={'position_id':pid,'signal_id':sid,'strategy':'NoDip','underlying':'NIFTY','expiry':str(expiry),'opened_at_utc':now_utc(),'lot_size':_lot(),'strikes':s['strikes'],'entry_prices':prices,'entry_sources':srcs,'quantities':qty,'entry_costs':costs,'paper_only':True};append('events',{'event_type':'OPEN','timestamp_utc':now_utc(),'position_id':pid,'position':pos});base.update(trade=True,status='PAPER_OPEN',position_id=pid)
  except Exception as e:append('errors',{'timestamp_utc':now_utc(),'strategy':'NoDip','error':type(e).__name__,'message':str(e)});base.update(trade=False,status='ENTRY_UNAVAILABLE')
 else: base['status']='GATE_PASS' if s['gate'] else 'GATE_FAIL'
